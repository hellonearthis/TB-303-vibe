class KO40SamplerEngine {
    ...
    async stopRecording() {
        ...
        try {
            const blob = await finished_recorder.stop();
            const raw_buffer = await blob.arrayBuffer();
            const decoded = await Tone.getContext().rawContext.decodeAudioData(raw_buffer);
            const slot = this.slots[this.selectedSlot];
            slot.buffer?.dispose();
            slot.buffer = new Tone.ToneAudioBuffer(decoded);
            slot.duration = decoded.duration;
            this.message('SAMPLE SAVED', `${this.slotName(this.selectedSlot)} · ${slot.duration.toFixed(1)}s`);
        } catch (error) {
